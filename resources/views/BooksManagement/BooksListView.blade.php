    ...
    if (dt_basic_table.length) {
        ...
        var table = dt_basic_table.DataTable({
            ajax: {
                ...
                type: 'GET',
                data: function (d) {
                    // Add selected filter value to the AJAX request data
        ...
        // On filter change, reload the datatable with new filter param
...
$(document).on('click', '.delete-btn', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/books-management/' + bookId,  // matches your destroy route
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function (response) {
                    if (response.success) {
                        ...
                        $('.datatables-basic').DataTable().ajax.reload(null, false); // refresh without reset page
                    }
                },
                error: function (xhr) {
